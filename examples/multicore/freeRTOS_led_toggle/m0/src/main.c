...
/* Configures and associates the uC RIT to the freeRTOS tick */
    ...
    /* Enable IRQ for M4 Core -- No la usamos en este ejemplo pero la dejo aca escrita. El funcionamiento
     * es el mismo solo que en el sentido contrario. */
...
/* Handler de la interrupcion del core remoto -- No es usada en el ejemplo pero la dejo escrita  */
...
static void ButtonPolling(void * a)
{
	...
	while(1)
	{
		if(Ciaa_Buttons_GetStatus() == tec)
		{
			...
			if(Ciaa_Buttons_GetStatus() == tec)
			{
				/* Enviamos evento para disparar la interrupcion en el core remoto */
				__SEV();
