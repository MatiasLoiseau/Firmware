...
teclasMEFStruct_t tecla_3 = {TEC3, teclaINICIAR_MEF};
teclasMEFStruct_t tecla_4 = {TEC4, teclaINICIAR_MEF};

uint32_t tickCount = 0;
uint32_t tickCountBetweenTecs = 0;
...
TASK(TickCounterBetweenTecs) {
   ++tickCountBetweenTecs;
   TerminateTask();
}
...
TASK(ReadTecs){
	...
	teclasMEFUpdate (&tecla_3);
	teclasMEFUpdate (&tecla_4);
	...
	if(tecla_2.teclaEstadoMEF == teclaJUST_DOWN){
		uartWriteString( UART_USB, "  Timer started by TEC2\n");
		SetRelAlarm(ActivateTickCounter, 0, 50);
	}
	if(tecla_3.teclaEstadoMEF == teclaJUST_DOWN){
		uartWriteString( UART_USB, "  Timer started by TEC3\n");
		SetRelAlarm(ActivateTickCounter, 0, 50);
	}
	if(tecla_4.teclaEstadoMEF == teclaJUST_DOWN){
		uartWriteString( UART_USB, "  Timer started by TEC4\n");
		SetRelAlarm(ActivateTickCounter, 0, 50);
	}
	...
	if(tecla_3.teclaEstadoMEF == teclaJUST_UP){
		uint8_t uartBuff[10];
		tickCount = tickCount * 50;	//Need to pass to MiliSeconds
		itoa( (int) tickCount, (char*)uartBuff, 10 );
		uartWriteString( UART_USB, uartBuff);
	...
	if(tecla_4.teclaEstadoMEF == teclaJUST_UP){
		uint8_t uartBuff[10];
		tickCount = tickCount * 50;	//Need to pass to MiliSeconds
		itoa( (int) tickCount, (char*)uartBuff, 10 );
		uartWriteString( UART_USB, uartBuff);
